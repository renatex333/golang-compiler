    ...
    def evaluate(self, symbol_table: SymbolTable):
        ...
        if (child_0_type == "INT" or child_0_type == "BOOL") and (child_1_type == "INT" or child_1_type == "BOOL"):
            if self.value == "+":
                ...
                raise Exception(
                    f"Invalid Operator Error: Operator {repr(self.value)} is not a valid operator between types {child_0_type} and {child_1_type}.")
    ...
    def evaluate(self, symbol_table: SymbolTable):
        ...
        if child_0_type == "INT" or child_0_type == "BOOL":
            if self.value == "+":
                ...
                raise Exception(
                    f"Invalid Operator Error: Operator {repr(self.value)} is not a valid unary operator fot type {child_0_type}.")
        else:
            raise Exception(
                f"Invalid Operator Error: Operator {repr(self.value)} is not a valid unary operator fot type {child_0_type}.")
...
class Assignment(Node):
    ...
    def evaluate(self, symbol_table: SymbolTable):
        ...
        if child_0_type != child_1_type:
            raise Exception(
                f"Invalid Type Error: Type {child_1_type} is not a valid type for a variable of type {child_0_type}.")
...
class VarDec(Node):
    ...
    def evaluate(self, symbol_table: SymbolTable):
        ...
        if len(self.children) > 1:
            child_1_value, child_1_type = self.children[1].evaluate(
                symbol_table)
            if child_1_type != self.value:
                raise Exception(
                    f"Invalid Type Error: Type {child_1_type} is not a valid type for a variable of type {self.value}.")
            symbol_table.set(
